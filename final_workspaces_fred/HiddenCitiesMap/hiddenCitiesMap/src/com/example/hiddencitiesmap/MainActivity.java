	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data)
	{
		...
		if (requestCode == CAM_REQUREST) {
			...
			if (resultCode == RESULT_OK) {
				Toast.makeText(this, "Image saved to:\n" + mLocalPathForFTP, Toast.LENGTH_LONG).show();
	...
	public void onLocationChanged(Location location)
	{
		if (markerLatLongList != null) {
			for (int l = 0; l < markerLatLongList.length; l++) {
				...
				if (distance < 10) {
					mVibrator.vibrate(1000);
					Toast.makeText(getApplicationContext(), "We are close to waypoint" + Integer.toString(l),
							Toast.LENGTH_SHORT).show();
				}
			}
		}
		if (mIsConnected) {
			mWSClient.send("map/" + location.getLatitude() + "/" + location.getLongitude());
		}
		...
		try {
			mWSClient = new WebSocketClient(new URI(mWsServerPath + mWSUserId)) {
				...
				public void onMessage(String aMessage)
				{
					...
					if (aMessage.equals("Attach Mouse Lines") || aMessage.equals("Tester")) {
