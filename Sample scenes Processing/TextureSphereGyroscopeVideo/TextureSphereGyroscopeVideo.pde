...
KetaiSensor sensor;
float gyroRotationX, gyroRotationY, gyroRotationZ;
...
void renderGlobe(PImage tex) {
  
  pushMatrix();
  translate(width * 0.5, height * 0.5, 2.0);
  pushMatrix();
  noFill();
  stroke(255,200);
  strokeWeight(2);
  ...
  pushMatrix();
  rotateX( direction );  
  rotateY( radians(270 - rotationX) );
  fill(200);
  noStroke();
  textureMode(IMAGE);  
  texturedSphere(globeRadius, tex);
  popMatrix();  
  popMatrix();
  rotationX += gyroRotationX;
  rotationY += gyroRotationZ;
