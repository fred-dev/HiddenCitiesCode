...
package processing.test.Vuforia;
...
import processing.test.Vuforia.utils.SampleMath;
import processing.test.Vuforia.utils.SampleUtils;
import processing.test.Vuforia.utils.Texture;
import processing.test.Vuforia.VideoPlayerHelper.MEDIA_STATE;
import processing.test.Vuforia.VideoPlayerHelper.MEDIA_TYPE;

// The renderer class for the VuforiaScene sample.
public class VuforiaSceneRenderer implements GLSurfaceView.Renderer
{
	private static final String	LOGTAG								= "VuforiaSceneRenderer";
	...
	int							videoPlaybackTextureID[]			= new int[VuforiaScene.NUM_TARGETS];
	...
	Vec2F						targetPositiveDimensions[]			= new Vec2F[VuforiaScene.NUM_TARGETS];
	...
	VuforiaScene				mVuforiaScene;

	// Needed to calculate whether a screen tap is inside the target
	Matrix44F					modelViewMatrix[]					= new Matrix44F[VuforiaScene.NUM_TARGETS];

	private Vector<Texture>		mTextures;
	private Vector<Texture>		mUtilityTextures;

	boolean						isTracking[]						= new boolean[VuforiaScene.NUM_TARGETS];
	boolean						wasTracking[]						= new boolean[VuforiaScene.NUM_TARGETS];
	MEDIA_STATE					currentStatus[]						= new MEDIA_STATE[VuforiaScene.NUM_TARGETS];

	// These hold the aspect ratio of both the video and the
	// keyframe
	float						videoQuadAspectRatio[]				= new float[VuforiaScene.NUM_TARGETS];
	float						keyframeQuadAspectRatio[]			= new float[VuforiaScene.NUM_TARGETS];

	public VuforiaSceneRenderer(VuforiaScene activity, VuforiaSession session)
	{

		mVuforiaScene = activity;
		vuforiaAppSession = session;

		// Create an array of the size of the number of targets we have
		mVideoPlayerHelper = new VideoPlayerHelper[VuforiaScene.NUM_TARGETS];
		mMovieName = new String[VuforiaScene.NUM_TARGETS];
		mCanRequestType = new MEDIA_TYPE[VuforiaScene.NUM_TARGETS];
		mSeekPosition = new int[VuforiaScene.NUM_TARGETS];
		mShouldPlayImmediately = new boolean[VuforiaScene.NUM_TARGETS];
		mLostTrackingSince = new long[VuforiaScene.NUM_TARGETS];
		mLoadRequested = new boolean[VuforiaScene.NUM_TARGETS];
		mTexCoordTransformationMatrix = new float[VuforiaScene.NUM_TARGETS][16];
		mVideoQuadTextureCoordsTransformed = new Vector<float[]>();

		for (int i = 0; i < VuforiaScene.NUM_TARGETS; i++) {
	...
	void initRendering()
	{
		Log.d(LOGTAG, "VuforiaScene VuforiaSceneRenderer initRendering");
		...
		//        keyframeQuadAspectRatio[VuforiaScene.STONES] = (float) mTextures
		//            .get(0).mHeight / (float) mTextures.get(0).mWidth;
		//        keyframeQuadAspectRatio[VuforiaScene.CHIPS] = (float) mTextures.get(1).mHeight
		//            / (float) mTextures.get(1).mWidth;
		for (int i = 0; i < mVuforiaScene.NUM_TARGETS; i++) {
	...
	void renderFrame()
	{
		...
		for (int tIdx = 0; tIdx < state.getNumTrackableResults(); tIdx++) {
			...
			// calculation
			for (int i = 0; i < mVuforiaScene.NUM_TARGETS; i++) {
				if (trackable.getName().equals(mVuforiaScene.mTargetsNames[i])
						|| trackable.getId() == mVuforiaScene.mTargetsIds[i]) {
					currentTarget = i;
					
					PApplet.println("found trackable with name "
							+ mVuforiaScene.mTargetsNames[i]);
				}
			}
			//            if (imageTarget.getName().compareTo("stones") == 0)
			//                currentTarget = VuforiaScene.STONES;
			//            else
			//                currentTarget = VuforiaScene.CHIPS;
...
//							mVuforiaScene.mSeekPosition[currentTarget] = 0;
//						
//						mVideoPlayerHelper[currentTarget].play(false,
//								mVuforiaScene.mSeekPosition[currentTarget]);
//						mVuforiaScene.mSeekPosition[currentTarget] = mVideoPlayerHelper[currentTarget].getCurrentPosition();
//					}
//
//					
//				}
...
//				}
			...
			SampleUtils.checkGLError("VuforiaScene renderFrame");
		}
	...
	boolean isTapOnScreenInsideTarget(int target, float x, float y)
	{
		...
		mVuforiaScene.getActivity().getWindowManager().getDefaultDisplay()
				.getMetrics(metrics);
			...
			float[] textureCoordMatrix)
	{
		...
		//        } else if (target == VuforiaScene.CHIPS)
		//        {
		//            tempUVMultRes = uvMultMat4f(
