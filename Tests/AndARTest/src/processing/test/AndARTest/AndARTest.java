package processing.test.AndARTest;
...
public class AndARTest extends PApplet {
	...
	public void setup() {
		...
		mRes = getResources();
		mARToolkit = new ARToolkit(mRes, getFilesDir());
		try {
			IO.transferFilesToPrivateFS(getFilesDir(),mRes);
		} catch (IOException e) {
			e.printStackTrace();
