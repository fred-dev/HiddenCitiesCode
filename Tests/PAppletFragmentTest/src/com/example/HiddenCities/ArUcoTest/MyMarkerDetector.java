...
public class MyMarkerDetector {
	...
	public double thresParam1, thresParam2;
	public thresSuppMethod thresMethod;
	public Mat grey, thres, thres2, hierarchy2;
	public Vector<MatOfPoint> contours2;
		
	public final static double MIN_DISTANCE = 10;
	 ...
	 * @param detectedMyMarkers output vector with the markers that have been detected.
	 * @param camMatrix --
	 * @param distCoeff --
	 * @param markerSizeMeters --
	 * @param frameDebug used for debug issues, delete this
	 */
	public void detect(Mat in, Vector<MyMarker> detectedMyMarkers, MyCameraParameters cp,//Mat camMatrix, Mat distCoeff,
			float markerSizeMeters, Mat frameDebug){
		Vector<MyMarker> candidateMyMarkers = new Vector<MyMarker>();
		...
		// to have a valid detectedMyMarkers vector longer
		Vector<MyMarker> newMyMarkers = new Vector<MyMarker>();
				...
				if(approxCurve.total()== 4){
					...
					if(Imgproc.isContourConvex(mat)){
						...
						if(minDistFound > MIN_DISTANCE){
							...
							candidateMyMarkers.add(new MyMarker(markerSizeMeters, p));
//							candidateMyMarkers.add(new MyMarker(markerSizeMeters));
//							candidateMyMarkers.lastElement().add(new Point(points[0],points[1]));
//							candidateMyMarkers.lastElement().add(new Point(points[2],points[3]));
//							candidateMyMarkers.lastElement().add(new Point(points[4],points[5]));
//							candidateMyMarkers.lastElement().add(new Point(points[6],points[7]));
						}
					}
				}
			}
		}// all contours processed, now we have the candidateMyMarkers
		int nCandidates = candidateMyMarkers.size();
		// sort the points in anti-clockwise order
		for(int i=0;i<nCandidates;i++){
			MyMarker marker = candidateMyMarkers.get(i);
		...
		for(int i=0;i<tooNearCandidates.size();i+=2){
			MyMarker first = candidateMyMarkers.get(tooNearCandidates.get(i));
			MyMarker second = candidateMyMarkers.get(tooNearCandidates.get(i+1));
		...
		for(int i=0;i<nCandidates;i++){
			if(toRemove.get(i) == 0){
				MyMarker marker = candidateMyMarkers.get(i);
				Mat canonicalMyMarker = new Mat();
				warp(in, canonicalMyMarker, new Size(50,50), marker.toList());
				marker.setMat(canonicalMyMarker);
				marker.extractCode();
				if(marker.checkBorder()){
					int id = marker.calculateMarkerId();
					if(id != -1){
						newMyMarkers.add(marker);
		...
		Collections.sort(newMyMarkers);
		toRemove.clear();
		for(int i=0;i<newMyMarkers.size();i++)
			toRemove.add(0);
		
		for(int i=0;i<newMyMarkers.size()-1;i++){
			if(newMyMarkers.get(i).id == newMyMarkers.get(i+1).id)
				if(newMyMarkers.get(i).perimeter()<newMyMarkers.get(i+1).perimeter())
					toRemove.set(i, 1);
				else
					toRemove.set(i+1, 1);
		}
		...
		for(int i=toRemove.size()-1;i>=0;i--)// done in inverse order in case we need to remove more than one element
			if(toRemove.get(i) == 1)
				newMyMarkers.remove(i);
		
		// detect the position of markers if desired
		for(int i=0;i<newMyMarkers.size();i++){
			if(cp.isValid())
				newMyMarkers.get(i).calculateExtrinsics(cp.getCameraMatrix(), cp.getDistCoeff(), markerSizeMeters);
		}
		detectedMyMarkers.setSize(newMyMarkers.size());
		Collections.copy(detectedMyMarkers, newMyMarkers);
	}
	 ...
	 */
	public void warp(Mat in, Mat out, Size size, List<Point> points){
