...
public class APWidgetContainer{
	...
	private static void enableGUI(PApplet argPApplet){
		argPApplet.getActivity().runOnUiThread(new EnableGUITask(argPApplet));
	}
...
//	private int index;
	...
	private void createLayout(PApplet pApplet){
		...
		pApplet.getActivity().getWindow().addContentView(
				scrollView,
				new ViewGroup.LayoutParams(
						ViewGroup.LayoutParams.FILL_PARENT,
						ViewGroup.LayoutParams.FILL_PARENT));
		
		layout = new RelativeLayout(pApplet.getActivity());
	...
	public void addWidget(APWidget pWidget) {
		pApplet.getActivity().runOnUiThread(new AddWidgetTask(pWidget));
	}
	
	public void removeWidget(APWidget pWidget) {
		pApplet.getActivity().runOnUiThread(new RemoveWidgetTask(pWidget));
	}
	...
	static class EnableGUITask implements Runnable{
		...
		public void run(){
			enableGUITaskPApplet.getActivity().getWindow().clearFlags(
					WindowManager.LayoutParams.FLAG_FULLSCREEN);
			enableGUITaskPApplet.getActivity().getWindow().setSoftInputMode(
	...
	public void release(){
		ViewGroup container = (ViewGroup) pApplet.getActivity().getWindow().getDecorView();
//		for (int i=0; i<views.size(); i++){
